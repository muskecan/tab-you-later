...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  $("#settings-btn").addEventListener("click", () => browser.runtime.openOptionsPage());
...
async function loadItems() {
  const resp = await browser.runtime.sendMessage({ action: "getItems" });
  allItems = resp.items || [];
  renderList();
}
...
async function handleUndo() {
  if (!undoToken) return;
  const resp = await browser.runtime.sendMessage({ action: "undoDelete", token: undoToken });
  hideUndoToast();
