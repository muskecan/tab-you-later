...
async function refreshEncryptionUI() {
  const unlockResp = await browser.runtime.sendMessage({ action: "isEncryptionUnlocked" });
...
async function handleEncryptAction() {
  ...
  if (mode === "enable") {
    const resp = await browser.runtime.sendMessage({ action: "enableEncryption", passphrase: pass });
    if (resp.success) {
    ...
    const resp = await browser.runtime.sendMessage({ action: "disableEncryption", passphrase: pass });
    if (resp.success) {
...
async function handleExport() {
  const resp = await browser.runtime.sendMessage({ action: "exportItems" });
...
async function onRemindersToggle() {
  const hasPerm = (await browser.runtime.sendMessage({ action: "hasNotificationPermission" })).granted;
  if (!hasPerm) {
