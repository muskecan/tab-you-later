...
async function save() {
  const resp = await chrome.runtime.sendMessage({ action: "saveSettings", settings: { ...settings } });
...
function enterEditMode(row, cat) {
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    if (!newName) return;
    const updateResp = await chrome.runtime.sendMessage({ action: "updateCategory", id: cat.id, name: newName, color: editColor });
...
async function clearAllData() {
  ...
  await chrome.storage.sync.remove("tylSettings");
